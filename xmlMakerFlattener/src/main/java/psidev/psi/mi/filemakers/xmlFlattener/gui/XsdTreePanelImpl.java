...
/**
 * 
 * This class overrides the abstract class AbstractXslTreePanel to provide a
...
//		Box validationBox = new Box(BoxLayout.X_AXIS);
//		validationBox.setBorder(new TitledBorder("Validate XML"));
...
//		buttonsPanel.add(validationBox);

		
		validateXml = new JCheckBox("validate XML");
		buttonsPanel.add(validateXml);
		JLabel validateText = new JLabel("(slower, required for");
		buttonsPanel.add(validateText);
		validateText = new JLabel("retrieving XML ids");
		buttonsPanel.add(validateText);
		validateText = new JLabel("e.g. normalized");
		buttonsPanel.add(validateText);
		validateText = new JLabel("PSI-MI XML 1.0)");
		buttonsPanel.add(validateText);
	...
	public class LoadDocumentListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			for (String error : xsdTree.xmlErrorHandler.getErrors()) {
				xsdTree.getMessageManager().sendMessage(error, MessageManagerInt.errorMessage);
			}
	 ...
	 * Used to parse the XML document using XML schema
	 */
	public class ValidateDocumentListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {

			
			((XsdTreeStructImpl) xsdTree).setValidateDocument(validateXml.isSelected());
						
			try {
				((XsdTreeStructImpl) xsdTree).loadDocument(((XsdTreeStructImpl) xsdTree).getDocumentURL());
				xsdTree.getMessageManager().sendMessage("XML document " + ((XsdTreeStructImpl) xsdTree).getDocumentURL() + " loaded.", MessageManagerInt.simpleMessage);
	...
	public class PrintFlatFileListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			if (((XsdTreeStructImpl) xsdTree).getDocument() == null) {
...
//			JOptionPane.showMessageDialog(new JFrame(), scrollPane);
	...
	public class SelectLineNodeListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			try {
				...
				((XsdTreeStructImpl) xsdTree).setLineXsdNode((XsdNode) xsdTree.tree
						.getLastSelectedPathComponent());

				((XsdTreeStructImpl) xsdTree)
						.setLineNode(((XsdTreeStructImpl) xsdTree).getLineXsdNode());
	...
	public class TreeMouseAdapter extends MouseAdapter {
		public void mouseClicked(MouseEvent e) {
			...
			if (((XsdTreeStructImpl) xsdTree).getElementFilters()
					.containsKey(selectedNode)) {
				filter
						.setText((String) ((XsdTreeStructImpl) xsdTree).getElementFilters()
								.get(selectedNode));
			} else {
	...
	public class XslTreeRenderer extends DefaultTreeCellRenderer {
		...
		public Component getTreeCellRendererComponent(JTree tree, Object value,
			...
			if (((XsdTreeStructImpl) xsdTree).getElementFilters().containsKey(node))
			...
			if (node == ((XsdTreeStructImpl) xsdTree).getLineXsdNode())
	...
	public void updatePreview() {
		...
		if (((XsdTreeStructImpl) xsdTree).getDocument() != null
				&& ((XsdTreeStructImpl) xsdTree).getLineXsdNode() != null) {
			String preview = "";
			...
			try {
				...
				preview += ((XsdTreeStructImpl) xsdTree)
						.getTitle(((XsdTreeStructImpl) xsdTree).getLineXsdNode())
						+ "\n";
			...
			if (displayExample.isSelected()) {
				try {
					((XsdTreeStructImpl) xsdTree).firstElement = true;
					if (((XsdTreeStructImpl) xsdTree).getDocument() != null
							&& ((XsdTreeStructImpl) xsdTree).getLineElements()
									.size() > 0) {
						if (exampleLine == null) {
							exampleLine = ((XsdTreeStructImpl) xsdTree)
									.marshallNode(
											((XsdTreeStructImpl) xsdTree).getLineXsdNode(),
											(Element) ((XsdTreeStructImpl) xsdTree).getLineElements()
													.get(0));
						}
						preview += exampleLine;
					}
	...
	public void associateFilter(XsdNode node) {
		String oldValue = (String) (((XsdTreeStructImpl) xsdTree).getElementFilters()
				.get(node));
		if (oldValue == null)
			oldValue = "";
		String value = (String) JOptionPane.showInputDialog(new JFrame(),
